...
def save_processed_data(X_df: pd.DataFrame, y_series: pd.Series, output_dir: str, prefix: str,output_type: str= 'csv'):
    """Saves the processed features and target to CSV or XLSX."""
    ...
    if output_type == 'xlsx':
        X_df.to_csv(os.path.join(output_dir, f"{prefix}_features.xlsx"), index=False)
        y_series.to_csv(os.path.join(output_dir, f"{prefix}_target.xlsx"), index=False)
        return
